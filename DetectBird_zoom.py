...
def capture_image_from_stream(capture, name):
    """
    ...
    output_image_path = output_folder + "/" +  name + "_" + datetime_format + ".jpg"

    cv2.imwrite(output_image_path, capture)


def tracking(camera, results, frame, speed, tolerance, target_label, capture,verbose=False):
    """
    ...
    if box.cls.nelement() > 0:
        ...
        if label == target_label:
            ...
            if (y2 - y1) < frame_shape_y/5 and zoom < 50:         
                camera.zoom()
                zoom+=1
            if (y2 - y1) > frame_shape_y/2 and zoom >0:         
                camera.dezoom()
                zoom-=1
    ...
    if all(target_present_history):
        start_time = time()
        #capture_image_from_stream(capture,"Image_Grand_Angle")
        end_time = time()
